...
goal_3_and_4_scatterplot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  ...
  labs(title = "Distribution of Goals 3 and 4",
       x = "SDG 3 Score",
       y = "SDG 4 Score",
       color="Regions")
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("RdPu", goal_10_score)(goal_10_score), label = mytext)
```
