...
```{r}
ggplot(data = sdr_data, aes(x= goal_8_score, y = goal_9_score)) +
  geom_point()


```
```{r}
ggplot(data = sdr_data, aes(x= goal_8_score, y = goal_9_score, color = goal_12_score)) +
  geom_point()

```
Interpretation: In the scatterplot above, the darkest points indicate a low score of SDG 12, Responsible Consumption and Production. It is interesting to see that those with a low SDG 12 score, also have a mid to high level SDG 8 and 9 score, which is Decent Work and Economic Growth (8) and Industry, Innovation, and Infrastructure (9). The light blue dots, which indicate a high level score of SDG 12 seems to have slightly high scores of SDG 8 and 9, but majority have low to mid level scores.

SDG 12 in a nutshell aims to promote resource efficiency, reduce waste, and encourage sustainable practices across all industries. SDG 8 and 9 focus on growing the economy, improving workforce, promoting innovation, and sustainable infrastructure. Knowing this, I think the visualization above is valuable as a foundation to create conversations on how to find a certain balance within these goals and solutions on how the world could be responsible consumers and producers, especially if various OECD countries rely on a consumerist behaviors.

Very fun challenge, thank youz!!!
...
Interpretation: Similar to the example, OECD countries are towards the higher end of SDG 6: Clean Water and Sanitation spectrum. But what is different about the SDG 6 visual is that there seems to be multiple countries in the mid-section of spectrum. Also, what stuck out to me was that Sub-Saharan African countries are slowl progressing to the higher-end of the Goal 6 spectrum, which makes me think they are starting to receive resources. 